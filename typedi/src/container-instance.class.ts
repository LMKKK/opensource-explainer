...
export class ContainerInstance {
  ...
  public readonly id!: ContainerIdentifier; // 容器的唯一标识，string或symbol类型

  /** Metadata for all registered services in this container. */
  /** 此容器中所有已注册的服务的metadata  */
   ...
   * 所有已注册的处理程序，@Inject()通过检查标志位 来进行注入
   **/
   ...
   * 标识此容器是否已经被弃置，在容器弃置后继续调用任何方法都将失败
   * 默认值false，此容器未被弃置，可正常使用
   */
  ...
  constructor(id: ContainerIdentifier) {
     ...
     */
    // handlers用来存放此类所有被@Inject()标记的属性
   ...
   * 重置容器，清空所有的已缓存的实例，
   * 策略：
   *    resetValue：只清空已缓存的实例，不清空已注册的服务
   *    resetServices：清空已缓存的实例，也清空已注册的服务
   */
  ...
  private initializeParams(target: Function, paramTypes: any[]): unknown[] {
    return paramTypes.map((paramType, index) => {
      ...
      // 若此参数被@Inject()修饰，直接从容器中获取此属性实例
      if (paramHandler) return paramHandler.value(this); // 返回属性实例，标明此参数已实例化
      // 构造参数中有@inject()指定的属性实例的依赖注入 end

      // 如果构造参数没有被@Inject()标注，则检查是否是原始类型，如果是原始类型，则返回undefined
      // 如果不是原始类型，则递归调用get方法，获取此类型的实例
  ...
  private applyPropertyHandlers(target: Function, instance: { [key: string]: any }) {
    this.handlers.forEach(handler => {
      if (typeof handler.index === 'number') return; // 构造参数的handler直接跳过，因为在此之前已经处理了
